...
public class TestMap : MonoBehaviour
{
    ...
    public GPSlocation userPosition = null;
	...
	private IEnumerator Start()
	{
       ...
       // GameObject.Find("Main Camera").GetComponent<Camera>()
        ...
        // 9 rue Gentil, Lyon
        map.CenterWGS84 = new double[2] { -0.131411 ,51.5221 };// { 4.83527, 45.76487 }; 
        ...
        if (area != null && finished == false)
        {
            ...
            {
                markerGO = Instantiate(go) as GameObject;
                map.CreateMarker<MarkerBehaviour>(poi.Name, new double[2] { poi.Longitude, poi.Latitude }, markerGO);
    ...
    void Update()
	{
        double longitude = Input.location.lastData.longitude;
        double latitude = Input.location.lastData.latitude;
        //map.CenterWGS84 = new double[2] { longitude ,latitude };
        if (destinationAngle != 0.0f)
		{
			Vector3 cameraLeft = Quaternion.AngleAxis(-90.0f, GameObject.Find("Main Camera").GetComponent<Camera>().transform.up) * GameObject.Find("Main Camera").GetComponent<Camera>().transform.forward;
			if ((Time.time - animationStartTime) < animationDuration)
			{
				...
				GameObject.Find("Main Camera").GetComponent<Camera>().transform.RotateAround(Vector3.zero, cameraLeft, angle - currentAngle);
				currentAngle = angle;
			}
			else
			{
				GameObject.Find("Main Camera").GetComponent<Camera>().transform.RotateAround(Vector3.zero, cameraLeft, destinationAngle - currentAngle);
